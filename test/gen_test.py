...
def main (argv):
    ...
    gc = json.load(fin)
    fin.close
    # keep some additional options  avaliable from template to target
    for part_template in gc['Participants'].keys():
        for index, port_template in enumerate(gc['Participants'][part_template]['Ports']):
            if port_template.get("switch", None) is not None:
                participants[part_template]['Ports'][index]["switch"] = port_template["switch"]
    ...
    for p in sorted(participants):
        ...
        for r in participants[p]['Ports']:  # routers
            ports.append(r['Id'])
        #print ports
        for switches in gc['RefMon Settings']['fabric connections'].keys():
            if len(ports) == 1:
                gc['RefMon Settings']['fabric connections'][switches][p] = ports[0]
            else:
                gc['RefMon Settings']['fabric connections'][switches][p] = ports
